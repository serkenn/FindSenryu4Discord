...
> このチャンネルでの自分の川柳検出を指定した分数だけ一時停止します。1〜1440分（最大24時間）。引数なしで残り時間を確認できます。
...
- **短歌検出** — 5-7-5-7-7の短歌パターンも自動検出。川柳より長いパターンが優先して検出される
- **自由律俳句ホワイトリスト** — 尾崎放哉・種田山頭火などの有名な自由律俳句をホワイトリストに収録。メッセージに含まれると作者名付きで検出される
...
- **自己タイムアウト** — `/timeout` でチャンネルごとに指定した分数だけ一時的に検出を停止
